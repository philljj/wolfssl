 ...
 *
 * To avoid a full copy of aad and text into buffer, instead
 * use AesGCM streaming API and incrementally pass in the aad
 * and then the plaintext (ciphertext).
 */
...
static int km_AesGcmEncrypt(struct aead_request *req)
{
    ...
    /* First get the associated data by starting a scatterwalk
     * over the first scatterlist that contains the aad.
     * Note: this might need to be improved to iteratively walk
     * several scatterlists to get all aad. */
...
static int linuxkm_test_cbc(void)
{
    ...
    /* Now the kernel crypto part: encrypt and decrypt the test vector
     * through kernel crypto API, and verify enc2 and dec2 match the
     * enc and dec from direct wolfCRYPT API calls. */
    ...
    pr_info("info: loaded wolfcrypt driver: %s\n", WOLFKM_CBC_DRIVER);
...
test_cbc_end:
    ...
    if (ret) {
        pr_err("error: linuxkm_test_cbc failed: %d\n", ret);
    }
...
test_cfb_end:
    ...
    if (ret) {
        pr_err("error: linuxkm_test_cfb failed: %d\n", ret);
    }
...
test_gcm_end:
    ...
    if (ret) {
        pr_err("error: linuxkm_test_gcm failed: %d\n", ret);
    }
