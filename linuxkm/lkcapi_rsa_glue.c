...
static struct akcipher_alg pkcs1_sha256 = {
    .base.cra_name        = "pkcs1(rsa,sha256)",
    .base.cra_driver_name = "pkcs1(rsa-wolfcrypt,sha256)",
    .base.cra_priority    = WOLFSSL_LINUXKM_LKCAPI_PRIORITY,
    .base.cra_module      = THIS_MODULE,
    .base.cra_ctxsize     = sizeof(struct km_RsaCtx),
    .sign                 = km_pkcs1_sign,
...
static int linuxkm_test_pkcs1(void)
{
    int rc = 0;
...
/**
 * Init the pkcs1 rsa ctx. The RNG is needed for padding
 * and blinding.
 * */
static int km_pkcs1_sha256_init(struct crypto_akcipher *tfm)
{
